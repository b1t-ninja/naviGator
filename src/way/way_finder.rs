...
impl<'a> WayFinder<'a> {
  pub fn new(args: &'a WayArgs) -> Self {
    Self {
      args,
      cwd: current_dir().expect("Failed to retrieve the parent process dir"),
    }
  }
  ...
  fn find_all_matching_dirs(&self) -> Vec<PathBuf> {
    let mut candidates = vec![self.cwd.clone()];
    ...
    for component in self.args.path.components() {
      ...
      for base in &candidates {
        let matches = self.find_all_sub_dirs(base).filter(|full_path| {
          full_path
            .file_name()
    ...
    candidates
      .into_iter()
      .filter_map(|p| p.strip_prefix(&self.cwd).ok().map(|p| p.to_path_buf()))
      .collect()
  }

  fn find_all_matching_fuzzy_dirs(&self) -> Vec<PathBuf> {
    let matcher = SkimMatcherV2::default();
    let mut candidates = vec![(0i64, self.cwd.clone())];
    ...
    for component in self.args.path.components() {
      ...
      for (total_score, base) in &candidates {
        let matches: Vec<(i64, PathBuf)> = self
          .find_all_sub_dirs(base)
          .filter_map(|full_path| {
            let name = full_path.file_name()?.to_str()?;
            matcher
              .fuzzy_match(name, &needle)
              .map(|score| (total_score + score, full_path))
          })
          .collect();
        next_candidates.extend(matches);
      }
    ...
    candidates
      .into_iter()
      .filter_map(|(_, p)| p.strip_prefix(&self.cwd).ok().map(|p| p.to_path_buf()))
      .collect()
  }
